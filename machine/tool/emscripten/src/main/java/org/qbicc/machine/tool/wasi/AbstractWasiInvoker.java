...
abstract class AbstractWasiInvoker implements MessagingToolInvoker {
    static final Path TMP = Paths.get(System.getProperty("java.io.tmpdir"));

    private static final Logger LOGGER = Logger.getLogger("org.qbicc.machine.tool.wasi");

    private final WasiToolChainImpl tool;
    private ToolMessageHandler messageHandler = ToolMessageHandler.DISCARDING;

    AbstractWasiInvoker(final WasiToolChainImpl tool) {
        this.tool = tool;
    }
    ...
    public WasiToolChainImpl getTool() {
        return tool;
    }
    ...
    public void invoke() throws IOException {
        OutputDestination errorHandler = OutputDestination.of(AbstractWasiInvoker::collectError, this, StandardCharsets.UTF_8);
