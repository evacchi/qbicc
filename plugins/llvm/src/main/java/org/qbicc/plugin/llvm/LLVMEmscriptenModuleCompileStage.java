...
public class LLVMEmscriptenModuleCompileStage implements Consumer<CompilationContext> {
    private final boolean isPie;
    private final boolean compileOutput;
    private final LLVMReferencePointerFactory refFactory;

    public LLVMEmscriptenModuleCompileStage(boolean isPie, boolean compileOutput, LLVMReferencePointerFactory refFactory) {
        this.isPie = isPie;
    ...
    public void accept(CompilationContext context) {
        ...
        if (compileOutput) {
            LLVMCompiler compiler = new LLVMEmscriptenCompilerImpl(context, isPie);
